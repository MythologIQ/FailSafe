...
**Standard feature**: `/ql-research` → `/ql-plan` → `/ql-audit` → `/ql-implement` → `/ql-substantiate` → `/ql-repo-release`

**Quick fix (L1)**: `/ql-plan` → `/ql-implement` → `/ql-substantiate`

**Diagnostics**: `/ql-status` → (routes to next action)

Full routing table: `.Codex/commands/references/ql-skill-routing.md`
