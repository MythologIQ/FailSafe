...
import { Plan, Blocker, Milestone } from "../../qorelogic/planning/types";
...
<body>
  ...
  private renderPlan(
    plan: Plan,
    progress: { completed: number; total: number; blocked: boolean },
  ): string {
    const pct =
      progress.total > 0
...
${milestones.length > 0 ? this.renderMilestones(milestones, completedMilestones) : ""}
  ...
  private renderMilestones(
    ...
    const milestoneItems = milestones
      .map((m) => {
        ...
        ${targetDate ? `<span class="milestone-date">${targetDate}</span>` : ""}
      </div>`;
      })
      .join("");

    return `<div class="milestones-section">
    <div class="section-title">Milestones (${completedCount}/${total} completed - ${pct}%)</div>
    <div class="milestones-list">${milestoneItems}</div>
  </div>`;
  }

  private renderPhases(plan: Plan): string {
    if (this.currentMode === "kanban") {
      return this.renderKanban(plan);
    }
    if (this.currentMode === "timeline") {
      return this.renderTimeline(plan);
    }
  ...
  private renderRoadSvg(plan: Plan): string {
    ...
    const rects = segments
      .map((s) => {
        ...
        const icon =
          s.phase.status === "blocked"
            ? '<text x="50%" y="60%" text-anchor="middle" font-size="10">B</text>'
            : "";
    ...
    const marker =
      curIdx >= 0
        ? `<circle cx="${(segments[curIdx].x + segments[curIdx].width / 2) * scale}" cy="35" r="4" fill="var(--vscode-charts-blue)"/>`
        : "";
...
${blockers
  .map(
    (
      b,
    ) => `<div class="blocker"><div class="blocker-title">${escapeHtml(b.title)}</div>
...
.milestones-section{margin-bottom:12px;padding:10px;border:1px solid var(--vscode-panel-border);background:#fff;border-radius:12px;box-shadow:0 4px 12px rgba(34,72,139,.07)}
.milestones-section .section-title{font-weight:700;font-size:11px;color:var(--vscode-descriptionForeground);margin-bottom:8px}
.milestones-list{display:flex;flex-direction:column;gap:6px}
.milestone-item{display:flex;align-items:center;gap:8px;font-size:11px;padding:6px 8px;background:var(--vscode-inputOption-activeBackground);border-radius:8px}
.milestone-item.completed{background:#e8f5e9}
.milestone-item.pending{background:#fff8e1}
.milestone-icon{font-size:12px}
.milestone-title{flex:1}
.milestone-date{font-size:10px;color:var(--vscode-descriptionForeground)}
