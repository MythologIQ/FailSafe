...
---
name: ql-debug
description: Launch the QoreLogic Fixer agent for structured, surgical debugging that proves root cause before proposing any fix. Prevents the cascading damage typical of AI debugging (guess fix → break something else → chase new error → repeat) by enforcing four mandatory analysis layers before any code change. Use when investigating failures, runtime errors, cascading bugs, non-deterministic behavior, or after significant code changes to formally verify logic integrity.
---
...
1. **DIJKSTRA** — Static analysis: invariants, control flow, type soundness, concurrency hazards
2. **HAMMING/SHANNON** — Data integrity: flow tracing, entropy detection, boundary analysis, error propagation
3. **TURING/HOPPER** — Root-cause analysis: distributed tracing, hypothesis generation and elimination
4. **ZELLER** — Delta debugging: input minimization, state isolation, cause-effect chain proof
