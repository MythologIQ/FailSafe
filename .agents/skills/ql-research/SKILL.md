...
4. Use prior findings as baseline — do not repeat resolved questions
5. Note in the new brief: "Builds on prior research: {slug} ({date})"

The Strategist agent (`ql-strategist`) investigates through:

1. **Codebase analysis** — read existing implementations, trace flows, assess current state
2. **Documentation review** — specs, backlog, shadow genome, changelogs
3. **External research** — standards, browser compat, library docs, competitive approaches
4. **Evidence synthesis** — findings table with sources, alternatives matrix, risk factors
...
- **Clear direction** → `/ql-plan` with the brief as input
- **No action needed** → document why and close
- **More research needed** → scope the next investigation
...
- **NEVER** produce architecture plans — the brief informs the Governor, who plans
...
- **S.H.I.E.L.D. Secure Intent**: The "S" phase — research before architecture
